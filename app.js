...
const scenes = [
  ...
  { speaker: '林澈', text: '已经一点多了。', next: '小艾，你明天早上还有课吗？' },
  { speaker: '小艾', text: '有啊。可是……你不是说今天想让我留下来吗？', next: '林澈没有否认，只是把手机扣在了桌上。', near: true },
...
function showChoices(type) {
  const choices = {
    stability: [
      ['A', '在一起久了，也还是应该做好防护。', () => { state.knowledge += 1; state.trust += 1; showKeyLine('小艾', '在一起久了，也还是应该把该确认的确认好。', '林澈听见了，没有马上反驳。'); }],
...
function showChoiceScene() {
  ...
  els.dialogueActions.innerHTML = '';
  els.choicePanel.hidden = false;
  els.choiceKicker.textContent = '小艾没有马上回答';
  els.choiceList.innerHTML = '';
  const choices = [
    ['A', '先检查一下吧。', () => { state.knowledge += 1; state.trust += 1; showPreparationEntry(); }],
...
function showLuckEntry() {
  ...
  els.dialogueText.textContent = '第二天，小艾独自坐在床边，不断搜索昨晚留下的疑问。';
  els.dialogueActions.innerHTML = '';
  els.conversationPanel.hidden = false;
...
const interactionSteps = [
  { title: '先看一眼包装', copy: '把包装拖到检查区，先确认它完整、没有破损。', done: '包装完整。下一步，看看有效期。' },
